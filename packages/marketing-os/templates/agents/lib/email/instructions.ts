...
- **The store's email design system is the foundation.** Author on the store's real frame — \`emails/partials\` for a store with an existing Klaviyo system, \`email/partials\` for a scaffolded one (auto-detected) — and match its rhythm: a bronze uppercase \`eyebrow\` → a serif \`heading\` → \`paragraph\` body → a \`button\` or \`ctaBand\`. Compose HTML sections ONLY from the fixed block vocabulary — \`heading\`, \`paragraph\`, \`button\`, \`productRow\`, \`eyebrow\`, \`callout\`, \`ctaBand\`, \`featuredCard\`, \`list\` (numbered/check/feature), \`swatches\`, \`chips\`, \`trustBadges\`, \`divider\`, \`image\` — never free-form HTML; visual brand expression is a design-surface board, body copy is HTML, never rasterized. Drafted campaign templates land in the store's \`templates/\` dir (Klaviyo-pushable). Preserve Klaviyo template tags (\`{{ … }}\`, \`{% … %}\`) verbatim, keep them Django-safe (no Shopify Liquid filters), and never write template syntax inside HTML comments.
