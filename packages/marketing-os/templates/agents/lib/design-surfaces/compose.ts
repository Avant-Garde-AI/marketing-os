/**
 * Vendored from marketing-os packages/design-surfaces @ 2026-07-18 (multi-board
 * WS2-R1 + compose fit-check) — replace with @avant-garde/design-surfaces when
 * published.
...
import type { BoardSpec, ComposeElement, ComposeSpec } from "./types";
  ...
  switch (el.type) {
    ...
    case "image": {
      const mediaId = ctx.addFileMedia(
        ...
        // Cast added while vendoring: this repo's TS lib types BlobPart as
          // ArrayBufferView<ArrayBuffer>, rejecting Uint8Array<ArrayBufferLike>.
