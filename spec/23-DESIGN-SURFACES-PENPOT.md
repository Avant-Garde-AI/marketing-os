...
> **Build status 2026-07-15: DS0–DS3 core BUILT + PROVEN END-TO-END on Arthaus** (marketing-os df9018e): local pinned stack (`infra/penpot/`), service-account bootstrap + tenant provisioning live, `packages/design-surfaces` (adapter, compose lane, export, CLI) with the **canary suite 8/8 green**, `compileDesignTokens` in brand-md (16/16). Proof artifact: `scripts/demo-arthaus.ts` — real DESIGN.md tokens → composed IG post → editable Penpot file → on-brand server-side PNG. Protocol findings + deferrals (production hosting, webhooks, template library stamping, OIDC, DS4/DS5): `docs/plans/design-surfaces/REVISIT.md`. Key as-built discovery: the export lane REQUIRES the §3 session-minting seam (exporter authenticates by session, not access token).

- **DS0 — Instance + tenancy spike. ✅ core (local)** — compose stack pinned + flag sets documented, service account, scripted idempotent team provisioning. Remaining: production host (REVISIT #1), OIDC wiring.
- **DS1 — Surface primitive + export. ✅ core** — adapter over the safe RPC set, `exportSurface` via the reverse-engineered `/api/export` protocol, canary suite v1, `mos_design_surfaces` migration drafted (app repo 9bde34a, unapplied). Remaining: webhook-driven `edited` detection (needs public callback).
- **DS2 — Compose lane. ✅** — `@avant-garde/design-surfaces` on `@penpot/library` (text content-tree synthesis, token-set filtering, image fills). Agent-composed documents open correctly in the editor. **SM1 unblocked.** OQ1 status: boards/rects/text/tokens/library-colors/images proven; components untested.
- **DS3 — Brand system push. ✅ compile + embed** — DESIGN.md → DTCG (brand-md), tokens travel inside composed files. Remaining: standalone template-library stamping + manifest-bump regeneration.
