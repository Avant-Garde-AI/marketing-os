...
| 22 | [22-BRAND-SOUL.md](./22-BRAND-SOUL.md) | Brand Soul & brand.md — the open `brand.md` spec (DESIGN.md's sibling: soul, not looks) with versioning + provenance; the 4-artifact manifest; the compose→deep-research→iterate→explore→publish pipeline; per-turn context engine; Brand console section; public Brand Portal + storefront agent discovery (END-TO-END LIVE on Arthaus 2026-07-11) |
| 23 | [23-DESIGN-SURFACES-PENPOT.md](./23-DESIGN-SURFACES-PENPOT.md) | Design Surfaces: the Penpot Core — platform-level design layer (decided 2026-07-15 over Canva/Figma): self-hosted multi-tenant Penpot wrapped in a `DesignSurface` primitive; agents compose headlessly file-first (`@penpot/library` → import-binfile), humans edit on the embedded "Design Studio" canvas, live co-creative sessions via the managed MCP (tab-bound, not headless); DESIGN.md → W3C-DTCG tokens + per-tenant brand template libraries (Stage 6 DISTRIBUTE); server-side export to the store repo. Domain-agnostic by rule — social/ads/offers/landing packs are consumers |
| 24 | [24-SOCIAL-MEDIA-AGENT.md](./24-SOCIAL-MEDIA-AGENT.md) | Social Media Agent — plans/maps/executes a store's social activity from the Brand Soul: derived calendars with provenance, artifacts in the store repo (files truth, DB index), console calendar view, post creative on spec-23 Design Surfaces (compose lane + embedded canvas), every publish a gated spec-20 Action executed by cron direct to channel APIs; performance readback seeds the next plan. First consumer of the Design Surface layer |
| 25 | [25-COMPOSABLE-BLOCKS-IMAGERY-AND-PACK-SCHEMAS.md](./25-COMPOSABLE-BLOCKS-IMAGERY-AND-PACK-SCHEMAS.md) | TRD — Composable Blocks, Resolved Imagery & Pack-Owned Schemas. Written from the shipped email agent: the creative vocabulary (not the model) was the quality ceiling, so blocks become a STORE-OWNED library the agent derives from the store's own templates and maintains — primitives stay platform code (a block composes primitives, never raw HTML), composed blocks become repo data. Imagery becomes a resolved capability behind declarative blocks (tiers: existing → composited mockup → generated scene; house rules; vision picks, human overrides). Storage splits: policy in the repo, compilations + ESP linkage in Supabase — which exposes the need for pack-owned namespaced schemas. Thesis + open decisions, written to be expanded into full specs |
| 27 | [27-AGENT-SESSIONS-AND-CHANGE-SETS.md](./27-AGENT-SESSIONS-AND-CHANGE-SETS.md) | **PROPOSAL** — Agent Sessions & Change Sets. Promotes the chat thread to a first-class session (list/resume, per-store not per-browser — the runtime's `/api/conversations` already ships it), and introduces `mos_change_sets` as the pack-agnostic unit of staged reviewable work: an index row, never the content, with the artifact staying on its pack's substrate (theme branch + preview theme / store-repo commit / ESP draft). Reviews becomes the one inbox across storefront, email, and social; applying stays inside the spec-20 Action gate so there is exactly one write path; sessions may own a `session/<id>` branch with approvals sequential within a session and independent across them. Four phases, P1 UI-only |
| 28 | [28-PLAYBOOKS-AND-FIRST-RUN.md](./28-PLAYBOOKS-AND-FIRST-RUN.md) | **PROPOSAL** — Playbooks & the First Run. The capability library as the onboarding path: numbered volumes (a vetted prompt with a name, already shipped in the OSS console) become the first-run sequence — define the brand → publish it publicly → put an offer live — each step producing the artifact the next consumes. Documents the open-core line on the offer engine: the authoring tools (`propose_offer`/`offer_review`/`offer_performance`) are MIT in the `@avant-garde/marketing-os` template; the storefront runtime, surface store, dark-pattern gate and `offer.activate` Action are the private hosted platform — and the ~400-line port that closes the gap for hosted stores |
| 32 | [32-OFFER-AGENT.md](./32-OFFER-AGENT.md) | **PROPOSAL** — The Offer Agent: consolidation, the pack, and the $15 wedge. Amends spec 14 Part II: the console's `Surfaces` section becomes **Offers** (the framework keeps the name in code; `Design Surfaces` keeps it in Design Studio), and the offer engine — today three diverging copies across three repos — is packaged as `packages/skills/offers` in the email/social pack shape (OfferRepo seam, repo artifacts, Actions, enable gate). Adds the parity scope that makes cancelling Alia/Wisepops possible (full-screen takeover, multi-step zero-party capture, ESP sync, announcement bar, exit-intent/teaser/targeting), the standing optimisation ritual, and the spec-31 ejection contract for offer stats (store-owned counters exposed to the pooled analytics agent over MCP, no silent fallback). Bundled at the $15 Starter tier — already wired in entitlements; the work is earning it |
