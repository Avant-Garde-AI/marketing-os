...
# Checks prerequisites, installs dependencies, validates environment.
...
if [ "$ERRORS" -gt 0 ]; then
  echo ""
  log_error "Prerequisites check failed with $ERRORS error(s). Fix the issues above and re-run."
  exit 1
fi

log_step "2/4  Installing agent dependencies"

if [ ! -d "$AGENTS_DIR" ]; then
  log_error "agents/ directory not found at $AGENTS_DIR"
  echo -e "  ${DIM}Run 'npx @avant-garde/marketing-os init' first.${NC}"
  exit 1
fi

cd "$AGENTS_DIR"
log_info "Running ${PM} install in agents/..."
$PM install
