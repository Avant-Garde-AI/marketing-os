...
export class PenpotClient {
  ...
  async rpcMultipart<T = unknown>(
    ...
    form.set(
      "file",
      // Cast added while vendoring: this repo's TS 5.7 lib types BlobPart as
      // ArrayBufferView<ArrayBuffer>, rejecting Uint8Array<ArrayBufferLike>.
      new Blob([file.data as Uint8Array<ArrayBuffer>], { type: file.contentType ?? "application/octet-stream" }),
      file.name,
    );
    const res = await fetch(`${this.baseUrl}/api/rpc/command/${command}`, {
      method: "POST",
